...
Http::FilterDataStatus FaultFilter::encodeData(Buffer::Instance& data, bool end_stream) {
  ...
  if (response_limiter_ != nullptr) {
    response_limiter_->writeData(data);
  }

  ASSERT(!end_stream); // fixfix
  return Http::FilterDataStatus::Continue;
}
...
void StreamRateLimiter::writeData(Buffer::Instance& incoming_buffer) {
  ...
  ENVOY_LOG(trace, "limiter: needed={} obtained={} to_write={}", tokens_needed, tokens_obtained,
            bytes_to_write);
